...
version_model="2.0.1"                                    # version of inputdata
version_input="version2.0.0" 
...
then
  ...
  then
    ...
    cd ~/ctsm_region/cime
    git status
    cd ~/ctsm_region/src/fates
    git status
    cd ~/ctsm_region/components/cism
    git status
    cd ~/ctsm_region/components/rtm
    git status
    cd ~/ctsm_region/components/mosart
    git status


    if [ ${SLURM_ARRAY_TASK_ID} == "0" ]
    then
     echo "create inputdata folder"
     mkdir -p /cluster/work/users/$USER/inputdata_${version_input}
    fi
    ...
    cp ${input_dir}/inputdata_${version_input}_${plotname[SLURM_ARRAY_TASK_ID]}.tar /cluster/work/users/$USER/inputdata_${version_input}
    cd /cluster/work/users/$USER/inputdata_${version_input}
...
######## Build the case and run the test
    ...
    ./xmlchange --file env_run.xml --id DIN_LOC_ROOT --val /cluster/work/users/$USER/inputdata_${version_input}/inputdata
    ./case.setup
    ./case.build
    ./.case.run
  fi
...
######## Run the case for longer time
  ...
  then
   ...
   ./xmlchange --file env_run.xml --id DIN_LOC_ROOT_CLMFORC --val /cluster/work/users/$USER/inputdata_${version_input}/inputdata/atm/datm7/GSWP3v1
    ./.case.run
  fi
